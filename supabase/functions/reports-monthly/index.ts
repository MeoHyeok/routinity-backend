...
export default {
  fetch: withSupabase({ auth: "user" }, async (req, ctx) => {
    ...
    const claudeRaw = await generateWithClaude(buildMonthlyClaudePrompt(stats, insights, breakdown));
    const claudeParsed = claudeRaw !== null ? extractSuggestedAction(claudeRaw) : null;
    const content = claudeParsed?.content ?? buildMonthlyTemplateReport(stats, insights, breakdown);
    ...
    const insert = await ctx.supabaseAdmin
      .from("ai_reports")
      .insert({ user_id: userId, period: "monthly", content, time_breakdown: timeBreakdownField, suggested_action: suggestedAction })
