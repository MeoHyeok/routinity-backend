...
export function buildTemplateReport(stats: GoalStat[], insights: InsightsResult | null = null): string {
  if (stats.length === 0) {
  ...
  return ["이번 주 루티니티 리포트", "", ...lines, ...patternSection].join("\n");
}

export function buildClaudePrompt(stats: GoalStat[], insights: InsightsResult | null = null): string {
  const lines = stats.map((s) => {
    const label = TARGET_TYPE_LABEL[s.target_type] ?? s.target_type;
