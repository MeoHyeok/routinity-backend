...
export function buildMonthlyTemplateReport(
  ...
  if (goalLines.length === 0 && (breakdownLines.length > 0 || rawActivityLines.length > 0)) {
    blocks.push(["목표를 설정하면 최근 한 달 기록을 목표 대비 달성률로도 볼 수 있어요."]);
  }
...
export function buildMonthlyClaudePrompt(
  ...
  const sections = ["다음은 한 사용자의 최근 한 달(30일) 루틴 기록이야.", ...goalLines];
  ...
  if (hasGoals) {
    ...
    sections.push(
      "",
      "이 사용자는 아직 목표를 설정하지 않았고, 그냥 그날그날 기록만 남기고 있어. 달성/미달성 같은 평가 없이, 최근 한 달간 기록된 활동을 있는 그대로 격려하는 톤으로 4~6문장으로 요약해줘. 과장하지 말고 실제 기록에 근거해서 구체적으로 작성해줘. 패턴 정보가 있다면 자연스럽게 녹여서 언급해줘.",
