...
export function buildDailyTemplateReport(scores: ScoreEntry[], dailyScore: number | null): string {
  if (scores.length === 0) {
    return "오늘은 설정된 목표가 없어 리포트를 생성할 수 없어요. 목표를 설정하면 오늘부터 리포트를 받아볼 수 있습니다.";
  }

  const lines = scores.map(describeScore);
  return ["오늘의 루티니티 리포트", "", `오늘의 루틴 점수: ${dailyScore}점`, "", ...lines].join("\n");
}

export function buildDailyClaudePrompt(scores: ScoreEntry[], dailyScore: number | null): string {
  const lines = scores.map(describeScore);
  return [
    ...
    ...lines,
    "",
    "이 데이터를 바탕으로 격려하는 톤의 한국어 일간 리포트를 2~3문장으로 작성해줘. 과장하지 말고 데이터에 근거해서 구체적으로 작성해줘.",
  ].join("\n");
}
