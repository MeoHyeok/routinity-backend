...
`goalsExistingBy` filters a goal list down to only goals whose `created_at` predates a given cutoff. `/insights` (added 2026-08-14, priority #3 in the post-launch feature list) uses it per-day when scoring its 28-day window — without it, a currently-set goal would retroactively score as "missed" every day before the user ever set it, which is exactly what happened on first deploy (see the `20260814080000_goals_created_at.sql` migration note) and skewed weekday averages/trend for anyone without a month of history. `_shared/insights.ts`'s `computeInsights` takes that per-day `daily_score` series and produces per-UTC-weekday averages (with best/worst) and a recent-7-days-vs-previous-7-days trend (`up`/`down`/`flat`, flat within a 3-point band to absorb rounding noise); both windows need at least one scored day or `trend` is `null`.

Covered by unit tests (38 cases total across `computeScores`, `computeDailyScore`, `goalsExistingBy`, `computeInsights`) plus live-endpoint verification.
