...
**Priority #4 — pattern-aware feedback** (added 2026-08-14): both report endpoints call `loadInsights` and splice `describeInsights`'s output (best/worst weekday, recent-vs-previous-week trend) into the template fallback (a "패턴 분석" section) and the Claude prompt (extra context lines), so feedback references patterns instead of only the current period's raw counts. This doesn't change either endpoint's response shape — `content` is still a plain string, just written with more context. Pattern loading is treated as an enrichment: a `loadInsights` failure is logged (`console.error`) and the report still generates without it rather than 500ing — core report generation shouldn't depend on the enrichment succeeding.
